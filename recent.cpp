...
Recent::Recent(QObject* parent, QString&& recentFilesKey)
    : QObject(parent)
...
void Recent::prependToRecentFiles(const QString& fileName) {
    ...
    //    MainWindow::instance()->documentWasModified();
}
...
void Recent::updateRecentFileActions() {
    ...
    int i = 0;
    for(; i < count; ++i) {
        const QString fileName = MainWindow::instance()->strippedName(recentFiles.at(i));
...
void Recent::openRecentFile() {
    if(const QAction* action = qobject_cast<const QAction*>(sender()))
        MainWindow::instance()->openFile(action->data().toString());
}
